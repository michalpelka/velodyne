...
namespace velodyne_rawdata
{
                             ...
                             VPointCloud &pc)
  {
    ...
    union fourbytes_bytes tmp;
    tmp.bytes[0]=pkt.data[1200];
    tmp.bytes[1]=pkt.data[1201];
    tmp.bytes[2]=pkt.data[1202];
    tmp.bytes[3]=pkt.data[1203];
    
    float f_tt = 1.0*tmp.uint / 1000000;
    ...
    for (int block = 0; block < BLOCKS_PER_PACKET; block++) {
      ...
      for (int firing=0, k=0; firing < VLP16_FIRINGS_PER_BLOCK; firing++){
        ...
        for (int dsr=0; dsr < VLP16_SCANS_PER_FIRING; dsr++, k+=RAW_SCAN_SIZE){
          ...
          float timeOffset =  (2*block+firing) * VLP16_FIRING_TOFFSET  + dsr*VLP16_DSR_TOFFSET;
          timeOffset = 1.0*timeOffset / 1000000 ; 
          velodyne_pointcloud::LaserCorrection &corrections =
          ...
          if ((azimuth_corrected >= config_.min_angle
            ...
            if (pointInRange(distance)) {
              ...
              point.z = z_coord;
              point.intensity = intensity;
              point.timestamp = f_tt + timeOffset;
              pc.points.push_back(point);
              ++pc.width;
            }
          }
